...
class PostSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Post
        fields = ('id', 'author', 'image', 'created_time', 'caption', 'post_comments_count', 'me_liked','post_likes_count')
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PostComment
        fields = ('id', 'author', 'comment', 'parent', 'created_time', 'replies', 'me_liked', 'likes_count','post')
    ...
    def get_me_liked(self, obj):
        ...
        if user.is_authenticated:
            return obj.likes.filter(author=user)
...
class CommentLikeSerilazer(serializers.ModelSerializer):
    id = serializers.UUIDField(read_only=True)
    author = UserSerializer(read_only=True)
